...
// MQTT 설정
// ============================================================
const char* MQTT_BROKER = "13.233.91.248";   // ⚠️ 실제 IP로
const int MQTT_PORT = 1883;
const char* MQTT_USER = "hearo";
const char* MQTT_PASS = "1234";              // ⚠️ 실제 비밀번호로
const char* MQTT_CLIENT_ID = "hearo-esp32-1"; // ⚠️ 노드마다 반드시 다르게!
const char* MQTT_TOPIC = "hearo/alert";
// ============================================================
// LED 핀 설정
// ============================================================
const int LED_RED = 25;     // 비상벨
const int LED_YELLOW = 26;  // 아기울음
const int LED_BLUE = 27;    // 도어락/노크
...
void onMqttMessage(char* topic, byte* payload, unsigned int length) {
  ...
  if (message == "비상벨소리") {
    ...
    Serial.println("🔐 방문자 알림! (파란 LED 천천히)");
    blinkLED(LED_BLUE, 500, BLINK_DURATION);
  } else if (message == "아기울음소리") {
    Serial.println("👶 아기 알림! (노란 LED 천천히)");
...
void connectMQTT() {
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect(MQTT_CLIENT_ID, MQTT_USER, MQTT_PASS)) {
      ...
      Serial.println(mqttClient.state());   // 4 or 5 = 인증 오류
      delay(5000);
    }
  }
}
